...
function nodeRadius(node: GraphNode): number {
  return NODE_RADIUS + Math.min(6, node.degree * 1.1);
}
...
export function InfluenceGraph({
  ...
  // The dashboard theme is class-based (next-themes); watch the <html> class
  // instead of pulling useTheme so the canvas also reacts to system changes.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const labelFont = getComputedStyle(container).fontFamily || 'sans-serif';
    ...
    function tick() {
      ...
      for (let i = 0; i < sim.length; i++) {
        ...
        // Weak pull toward the chronological slot and vertical center.
        const targetX = EDGE_PAD + a.chrono * Math.max(1, width - EDGE_PAD * 2);
        a.vx += (targetX - a.x) * 0.045 * alpha;
      ...
      for (const n of sim) {
        ...
        n.y = Math.min(height - LABEL_GAP - 14, Math.max(r + 4, n.y));
      }
    ...
    function draw() {
      ...
      const haloColor = isDark ? 'rgba(10, 8, 4, 0.65)' : 'rgba(250, 247, 242, 0.75)';
      ...
      for (const link of linkPairs) {
        ...
        const x2 = p.x - ux * (nodeRadius(p) + 6);
        const y2 = p.y - uy * (nodeRadius(p) + 6);
      ...
      for (const n of sim) {
        ...
        const emphasized = highlight ? inHighlight : filter ? inFilter : true;
        ...
        ctx!.fillStyle = emphasized ? baseText : dimText;
        ctx!.globalAlpha = 1;
        ctx!.fillText(t(n.philosopher.name, locale), n.x, n.y + r + 5);
      }
    }
  ...
  return (
    <div className="space-y-3">
      {/* Legend: one chip per school; click to spotlight its members. */}
      <div
        ...
        {schools.map((school) => {
          ...
          return (
            <button
              ...
              aria-pressed={active}
              className={`flex items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] uppercase tracking-wider transition-all ${
                active
            ...
            >
              <span
                ...
                style={{ backgroundColor: active ? '#fff' : school.accent }}
              />
      ...
      <div
        ref={containerRef}
        className="relative h-[440px] w-full overflow-hidden rounded-2xl border border-gold-500/25 bg-white/40 shadow-card dark:bg-midnight-800/40 sm:h-[500px] md:h-[560px]"
      >
        <canvas
          ref={canvasRef}
        ...
        />
        <p className="pointer-events-none absolute right-3 top-3 hidden text-[10px] uppercase tracking-[0.2em] opacity-50 sm:block">
      ...
      <p className="text-center text-[11px] uppercase tracking-[0.2em] opacity-50 sm:hidden">
